			       ...
			       unsigned int connect_timeout, const char *socket_name)
{
  ...
  locked = true;
  if (mysql_real_connect(&mysql,host,user,passwd,db,port,socket_name,0))
  {
    locked = false;
    Success = is_connected = true;
  }
...
{
  Success = !(mysql_select_db(&mysql, db));
  if (throw_exceptions && !Success) throw MysqlBadQuery(error());
  else return Success;
}
//--------------------------------------------------------------------------

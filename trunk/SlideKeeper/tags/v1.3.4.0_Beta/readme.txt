...
====================
Jan 23, 2005 v1.3.4
  Fixed the edit forms so they would close properly when using x.
  Update slide form now updates the drop down boxes when changed.
  Fixed entry date so it would show up.
  Fixed update form so the tab button would not delete data in drop down menu's.
  Fixed the find form so it doesn't crash with blank data.
  Fixed update form to not change approved user when moving to next/previous.
  Fixed update form to not crash when updating data.
  Fixed some formatting issues and added user and password to config file.

...
void __fastcall TUpdForm::upd_btnClick(TObject *Sender){
    ...
    sql = sql + "', tb_MaterialKey = '" + SQLParse(material)
        + "', tb_PDateKey = '" + SQLParse(period) + "', tb_Edate = STR_TO_DATE('"
        + SQLParse(EDate_date->Text) + "', '%m/%d/%Y %h:%i:%s %p')";
    ...
    sql = sql + "' AND tb_EDate LIKE STR_TO_DATE('" + SQLParse(spres->EDate) +
    "', '%m/%d/%Y %h:%i:%s %p')";
